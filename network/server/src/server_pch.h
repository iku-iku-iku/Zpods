...
namespace zpods{
    ...
    public:
        DbHandle() {
            ...
            rocksdb::Status status = rocksdb::DB::Open(options, "zpods_server_db", &db);
            assert(status.ok());
        }
        ...
        auto put_username2password_hash(const std::string& username, const std::string& password_hash) {
            return Put(username_key(username), password_hash);
        }

        auto get_password_hash_by_username(const std::string& username, std::string* password_hash) {
...
inline void put_token(const std::string& token, int ttl = 60 * 60 * 24 * 7) {
    ...
    let expiry = std::chrono::system_clock::to_time_t(now + std::chrono::seconds(ttl));
...
inline bool is_token_valid(const std::string& token) {
    ...
    rocksdb::Status s = zpods::DbHandle::Instance().Get(TTL + token, &expiry_value);
    if (!s.ok()) {
        return false;
    }
