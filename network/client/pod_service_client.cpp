...
zpods::Status PodServiceClient::UploadPod(const std::string &pod_path) {
    ...
    std::unique_ptr<ClientWriter<Pod>> writer(stub_->UploadPod(&context, &upload_status));

    Pod pod;
    let pod_name = std::filesystem::path(pod_path).filename().string();
    let pods_name = std::filesystem::path(pod_path).parent_path().filename().string();
    spdlog::info("uploading {}/{}", pods_name, pod_name);
    pod.set_pod_name(pod_name);
    pod.set_pods_name(pods_name);

    {
        let status = zpods::DbHandle::Instance().get_cached_token(pod.mutable_token());
