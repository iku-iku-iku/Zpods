...
TEST(CryptoTest, EncryptDecrypt1)
{
    ...
    // 加密
    ciphertext_len = zpods::raw_encrypt(
        plaintext, (int)strlen((char*)plaintext), key, iv, ciphertext);
    ASSERT_GT(ciphertext_len, 0);

    // 解密
    decryptedtext_len =
        zpods::raw_decrypt(ciphertext, ciphertext_len, key, iv, decryptedtext);
...
{
    std::string src_path = fmt::format("{}/{}", zpods::test_data_path(),
                                       "single/man_pthreads.txt");
    std::string dst_path =
        fmt::format("{}/{}", zpods::temp_path(), "single/man_pthreads.txt");
    std::string key = std::to_string(rand());
    ...
    {
        let status =
            zpods::encrypt_file(src_path.c_str(), dst_path.c_str(), config);
        EXPECT_EQ(status, zpods::Status::OK);
    }

    // 验证加密后的文件是否与原始文件不同
    EXPECT_FALSE(
        zpods::fs::is_same_content(src_path.c_str(), dst_path.c_str()));
    // 测试解密
    {
        let status =
            zpods::decrypt_file(dst_path.c_str(), dst_path.c_str(), config);
        EXPECT_EQ(status, zpods::Status::OK);
    }
    // 验证解密后的文件是否与原始文件相同
