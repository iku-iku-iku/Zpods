...
void compress_decompress_test_helper(std::span<byte> src)
{
    ...
    spdlog::info("[raw] {}, [lz77] {} [lz77&huffman] {}", src.size(), dst_size1,
                 dst_size2);
...
TEST(CompressTest, FileCompressDecompress)
{
    ...
    let status2 =
        decompress_file(dst_compressed.c_str(), dst_decompressed.c_str());
    ...
    spdlog::info("src_file_size: {}, compressed_file_size: {}, "
                 "decompressed_file_size: {}",
                 src_file_size, compressed_file_size, decompressed_file_size);
    spdlog::info("compress ratio: {}",
                 (double)compressed_file_size / (double)src_file_size);
