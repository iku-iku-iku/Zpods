...
TEST(FsTest, BasicTest)
{
    ...
    EXPECT_STREQ(fs::relative("/home/a.txt", "/home/"), "a.txt");
    EXPECT_DEATH(fs::relative("a.txt", "a.txt"),
                 "Assertion failed: `is_directory\\(base\\)`");
...
//     let path1 = fs::path(test_data_path()) / fs::path("filter") /
//     fs::path("dir2"); let path2 = path1 / fs::path("sub");
...
TEST(FsTest, FileCollectorWithTypes)
{
    ...
    filtered_paths_expect_eq({"soft_lint_to_man_fork.txt"}, filter_path,
                             fs::FilesFilter{.types = {fs::FileType::symlink}});
}
...
{
    let filter_path = fs::path(test_data_path()) / fs::path("filter");
    filtered_paths_expect_eq({"main.c", "man_printf.txt"}, filter_path,
                             fs::FilesFilter{
...
TEST(FsTest, FileCollectorWithNames)
{
...

    {
        ...
        filtered_paths_expect_eq(
            ...
            filter_path,
            {.types = {fs::FileType::regular}, .re_list = {"man"}});
    }
}
