...
zpods::Status zpods::User::download_file(const char* hdfs_path,
                                         const char* local_dir)
{
    let_mut client =
        Client::connect_server(); // 替换为您的Spring Boot应用地址和端口
    ...
    if (res && res->status == 200)
    {
        // 保存文件或处理文件数据
        let_mut output_file =
            fs::open_or_create_file_as_ofs(local_path.c_str(), fs::ios::binary);
...
zpods::Status zpods::download_file(const char* hdfs_path,
                                   const char* local_path)
{
    ...
    if (res && res->status == 200)
    {
        // 保存文件或处理文件数据
        let_mut output_file =
            fs::open_or_create_file_as_ofs(local_path, fs::ios::binary);
        spdlog::info("FILE SIZE: {}", res->body.size());
        output_file.write(res->body.data(), res->body.size());
        output_file.close();
    }
    ...
    {
        std::cout << "Failed to download file! HTTP Status: " << res->status
                  << std::endl;
        return Status::ERROR;
    }
