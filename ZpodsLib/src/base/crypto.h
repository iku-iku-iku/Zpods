 ...
 */
int raw_encrypt(p_cbyte plaintext, int plaintext_len, p_cbyte key, p_cbyte iv,
                p_byte ciphertext);

/*
 * @brief C++ style API encapsulation of raw_encrypt for easier use
 */
auto encrypt(std::string_view plaintext, std::string_view key,
 ...
 */
Status encrypt_file(const char* src_path, const char* dst_path,
 ...
 */
int raw_decrypt(p_cbyte ciphertext, int ciphertext_len, p_cbyte key, p_cbyte iv,
                p_byte plaintext);

/*
 * @brief C++ style API encapsulation of raw_decrypt for easier use
 */
auto decrypt(std::string_view ciphertext, std::string_view key,
             std::string_view iv) -> std::optional<std::string>;
 ...
 * @return OK if success, otherwise return the error code
 */
Status decrypt_file(const char* src_path, const char* dst_path,
                    const CryptoConfig& config);
} // namespace zpods
