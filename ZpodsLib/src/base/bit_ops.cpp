...
zpods::BitStream::BitStream(char* ptr)
    : base_ptr_(reinterpret_cast<unit_pointer>(ptr))
{
...
void zpods::BitStream::append_bit(unit_type bit)
{
    ...
    // clear bit
    *(base_ptr_ + write_bit_pos_ / BITS_PER_UNIT) &=
        ~((unsigned)1 << (unsigned)(write_bit_pos_ % BITS_PER_UNIT));
    // set bit
    *(base_ptr_ + write_bit_pos_ / BITS_PER_UNIT) |=
        ((unsigned)(bit & 1) << (unsigned)(write_bit_pos_ % BITS_PER_UNIT));
    write_bit_pos_++;
}
...
{
    unit_type bit = *(base_ptr_ + read_bit_pos_ / BITS_PER_UNIT) >>
                        (read_bit_pos_ % BITS_PER_UNIT) &
                    1;
    read_bit_pos_++;
    return bit;
}
