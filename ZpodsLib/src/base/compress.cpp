...
template <typename T>
std::pair<T, Code> read_with_dict(BitStream& bs,
                                  const std::unordered_map<Code, T>& dict)
{
    size_t bits = 0;
    size_t len = 0;
    while (true)
...
class SearchBuffer
{
  public:
    ...
    std::pair<size_t, size_t> search(p_cbyte buf, size_t cur_offset,
                                     size_t buf_len)
    {
        size_t search_window_start = cur_offset > SEARCH_WINDOW_SIZE
                                         ? cur_offset - SEARCH_WINDOW_SIZE
                                         : 0;
...

        {
            ...
            for (size_t match_start = search_window_start;
                 match_start < cur_offset; match_start++)
            {

                for (match_len = 0; match_len + match_start < cur_offset &&
                                    match_len < LOOK_AHEAD_WINDOW_SIZE - 1 &&
                                    match_len < remain_len;
                     match_len++)
                {
                    if (buf[match_start + match_len] !=
                        buf[cur_offset + match_len])
                    {
                        break;
                    }
                }
...
std::pair<size_t, std::unique_ptr<byte[]>>
zpods::compress(std::span<byte> src_rng)
{
    ...
    if (!HuffmanEnabled)
    {
        ...
        for (size_t i = 0; i < offsets.size(); i++)
        {
            ...
            if (offset == 0 && len == 0)
            {
                ...
                spdlog::debug("(ENCODE) OFFSET: {}, LEN: {}, LITERAL: {:c}",
                              offset, len, literal);
            }
            else
            {
    ...
    else
    {
        ...
        for (size_t i = 0; i < offsets.size(); i++)
        {
            ...
            if (offset == 0 && len == 0)
            {
                ...
                spdlog::debug("(ENCODE) OFFSET: {}, LEN: {}, LITERAL: {}",
                              offset_code, len_code, literal_code);
            }
            else
            {
                spdlog::debug("(ENCODE) OFFSET: {}, LEN: {}", offset_code,
                              len_code);
            }
        }
    }
...
std::pair<size_t, std::unique_ptr<byte[]>> zpods::decompress(p_cbyte src)
{
        ...
        while (true)
        {
            ...
            if (literal_mark(offset, len))
            {
                let[literal, literal_code] =
                    read_with_dict(src_bs, literal_dict);
                literals.push_back(literal);
                spdlog::debug("(DECODE) OFFSET: {}, LEN: {}, LITERAL: {}",
                              offset_code, len_code, literal_code);
            }
            else
            {
                spdlog::debug("(DECODE) OFFSET: {}, LEN: {}", offset_code,
                              len_code);
            }
        }
    }
    ...
    for (size_t i = 0; i < offsets.size(); i++)
    {
        ...
        if (literal_mark(offset, len))
        {
            ...
            spdlog::debug("(DECODE) OFFSET: {}, LEN: {}, LITERAL: {:c}", offset,
                          len, literal);
        }
        else
        {
            spdlog::debug(
                "(DECODE) OFFSET: {}, LEN: {}, WORDS: {}", offset, len,
                std::string_view(
                    (const char*)(dst_bs.write_byte_ptr() - offset), len));
...
Status zpods::compress_file(const char* src_path, const char* dst_path)
{
    ...
    let hash =
        std::hash<std::string_view>()({(char*)compressed.get(), dst_size});
...
template std::pair<size_t, std::unique_ptr<byte[]>>
    zpods::compress<LZ77>(std::span<byte>);

template std::pair<size_t, std::unique_ptr<byte[]>>
    zpods::compress<LZ77 | Huffman>(std::span<byte>);

template std::pair<size_t, std::unique_ptr<byte[]>>
zpods::decompress<LZ77>(p_cbyte src);

template std::pair<size_t, std::unique_ptr<byte[]>>
zpods::decompress<LZ77 | Huffman>(p_cbyte src);
