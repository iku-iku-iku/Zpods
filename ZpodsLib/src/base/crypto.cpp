...
int zpods::raw_encrypt(p_cbyte plaintext, int plaintext_len, p_cbyte key,
                       p_cbyte iv, p_byte ciphertext)
{
    EVP_CIPHER_CTX* ctx;
    int len;
    int ciphertext_len;
...
int zpods::raw_decrypt(p_cbyte ciphertext, int ciphertext_len, p_cbyte key,
                       p_cbyte iv, p_byte plaintext)
{
    {
        let hash = std::hash<std::string_view>()(
...
auto zpods::encrypt(std::string_view plaintext, std::string_view key,
                    std::string_view iv) -> std::optional<std::string>
{
    ...
    if (!(ciphertext_len = raw_encrypt(
              reinterpret_cast<p_cbyte>(plaintext.data()), plaintext_len,
              reinterpret_cast<p_cbyte>(key.data()),
              reinterpret_cast<p_cbyte>(iv.data()), &cipher_buf[0])))
    {
        return {};
    }
                    ...
                    std::string_view iv) -> std::optional<std::string>
{
    ...
    if (!(plaintext_len =
              raw_decrypt(reinterpret_cast<p_cbyte>(ciphertext.data()),
                          ciphertext_len, reinterpret_cast<p_cbyte>(key.data()),
                          reinterpret_cast<p_cbyte>(iv.data()), &plain_buf[0])))
    {
        return {};
    }
