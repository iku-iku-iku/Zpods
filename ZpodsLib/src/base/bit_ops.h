...
namespace zpods {
    ...
    public:
        ...
        p_byte write_byte_ptr() { return reinterpret_cast<p_byte>(base_ptr_) + write_bit_pos_ / BYTE_BITS_LEN; }

        [[nodiscard]] auto floor_write_bytes() const { return write_bit_pos_ / BYTE_BITS_LEN; }

        [[nodiscard]] auto write_len_in_bytes() const { return ceil_div(write_bit_pos_, BYTE_BITS_LEN); }

        unit_pointer get_buf_ptr_() { return bytes_buf_.get(); }

        auto take_buf() -> std::unique_ptr<byte[]>&& { return std::move(bytes_buf_); }
