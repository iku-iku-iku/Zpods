                                   ...
                                   std::string& bytes)
{
    ...
    // read content
    bytes = {(std::istreambuf_iterator<char>(ifs)),
             (std::istreambuf_iterator<char>())};

    return Status::OK;
}

void zpods::calculate_checksum(byte (&checksum)[CHECKSUM_SIZE],
                               std::span<byte> bytes)
{
...
Status zpods::process_origin_zpods_bytes(
    const char* path, BackupConfig& config,
    const std::function<Status(std::string&)>& func)
{
    ...
    {
        let current_password_verify_token =
            std::string{as_c_str(header.password_verify_token),
    ...
    if (config.crypto_config)
    {
        ...
        let plain_text = decrypt({as_c_str(bytes.data()), bytes.size()},
                                 conf->key_, conf->iv_);
...
// Status zpods::foreach_file_in_zpods_file(const char *path, ref_mut
// <BackupConfig> config,
//                                          ref <std::function<Status(ref <
//                                          fs::zpath > , std::string_view)>>
//                                          func) {
...
void zpods::calculate_password_verify_token(PodHeader& header,
                                            const std::string& password)
{
    let_mut cipher =
        zpods::encrypt({as_c_str(header), sizeof(header)}, password,
                       {as_c_str(header.iv), sizeof(header.iv)});

    memcpy(header.password_verify_token, cipher->c_str(),
           sizeof(header.password_verify_token));
}

Status zpods::foreach_pea_in_pod_bytes(
    byte* bytes, const std::function<Status(const PeaHeader&)>& func)
{
    let_mut p = bytes;
    PeaHeader* header;
